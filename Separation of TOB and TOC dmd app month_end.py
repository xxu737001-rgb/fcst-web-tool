...
# 当月和次月使用提货跟进表，PSI只提供后续四个月的需求。
DEMAND_COLUMNS = ["m_2", "m_3", "m_4", "m_5"]
...
def read_workbooks(files):
    ...
    delivery_1 = pd.read_excel(files["delivery_1"])
...
def prepare_delivery(
    ...
    *,
    ...
    safety_stock_column = f"{demand_column}安全库存"
    ...
    coerce_numeric_columns(
        delivery,
        [demand_column, safety_stock_column],
        workbook_name,
    )
    ...
    if unknown_customer_types:
        warnings.append(
            ...
            f"这些类型的{demand_column}未计入预计总提货。"
        )
    ...
    safety_stock = (
        ...
        .rename(
            columns={
                safety_stock_column: f"{prefix}_安全库存"
            }
        )
    )
...
def main() -> None:
    ...
    with action_col:
        psi_file = st.file_uploader(
            "PSI文件",
            type=["xlsx", "xlsm", "xls"],
            help="必需列：SKU、品牌、m_2、m_3、m_4、m_5。",
        )

        delivery_file = st.file_uploader(
            "当月提货跟进表",
            type=["xlsx", "xlsm", "xls"],
            help="必需列：SKU、品牌、客户类型、m_0、m_0安全库存。",
        )

        delivery_1_file = st.file_uploader(
            "次月提货跟进表（月末版）",
            type=["xlsx", "xlsm", "xls"],
            help="必需列：SKU、品牌、客户类型、m_1、m_1安全库存。",
        )
